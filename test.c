...
int main (int argc, char *argv []) 
{
    ...
    FILE * driver;
    driver = fopen("/dev/carchardev","r");
    char *buffer;
    buffer = (char*)malloc(100);
    while(1){
        int x = fread(buffer,1,1,driver);
        if(x>0){
            if(!strcmp("u",buffer)){
                SetCmdMode();
                lcd_byte(0x01);  // clear screen
                ...
                delay(3);
                SetChrMode();
                lcd_text("You pushed DOWN");
            }
            else if(!strcmp("l",buffer)){
                SetCmdMode();
                lcd_byte(0x01);  // clear screen
                lcd_byte(0x02);
                delay(3);
                SetChrMode();
                lcd_text("You pushed LEFT");
            }
            else if(!strcmp("r",buffer)){
                SetCmdMode();
                lcd_byte(0x01);  // clear screen
                lcd_byte(0x02);
                delay(3);
                SetChrMode();
                lcd_text("You pushed RIGHT");
            }
